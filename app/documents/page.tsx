...
export default function DocumentViewer() {
    ...
    if (printWindow) {
      printWindow.document.write(`
        <html>
          ...
          <body>
            ${filteredDocuments.map((doc) => `<img src="${doc.url}" alt="${doc.title}" />`).join("")}
          </body>
        </html>
      `)
  ...
  const downloadAllDocuments = () => {
    ...
    alert("In a production environment, this would download all documents as a ZIP file.")
  }
        ...
        <div className="container mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-blue-100">Contract No. SCOC/SPLS/WRKS/CKVS/24-25/10/1</p>
            </div>
        ...
        <div className="container mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex space-x-2">
              ...
              <Button
                variant={activeCategory === "identification" ? "default" : "outline"}
                size="sm"
      ...
      <main className="flex-1 container mx-auto p-4 md:p-6 flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-64 bg-white rounded-lg shadow-md p-4 h-[200px] md:h-auto overflow-y-auto">
          ...
          <div className="space-y-2">
            {filteredDocuments.map((doc, index) => (
              <div
                key={doc.id}
                className={`flex items-center p-2 rounded cursor-pointer hover:bg-gray-100 ${currentDocIndex === index ? "bg-blue-50 border border-blue-200" : ""}`}
        ...
        <div className="flex-1 bg-white rounded-lg shadow-md flex flex-col">
          ...
          <div className="border-b border-gray-200 p-3 flex items-center justify-between bg-gray-50 rounded-t-lg">
            ...
            <div className="flex items-center space-x-2">
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="outline" size="icon" onClick={downloadAllDocuments}>
          ...
          <div className="border-t border-gray-200 p-3 bg-gray-50 rounded-b-lg">
            <h3 className="font-medium text-gray-800">{currentDocument.title}</h3>
            <p className="text-sm text-gray-600">
              ...
              {currentDocument.id === 5 && "Inspection, testing, and packaging requirements for the goods"}
              {currentDocument.id === 6 &&
                "Delivery terms, insurance requirements, and warranty information (3-year warranty)"}
              {currentDocument.id === 7 && "Payment terms and schedule for the contract"}
              {currentDocument.id === 8 && "Contract amendments and assignment clauses"}
              {currentDocument.id === 9 && "Force majeure and dispute resolution clauses"}
              {currentDocument.id === 10 && "Contract clauses regarding applicable law and notices"}
              {currentDocument.id === 11 && "Contract signatures from all parties"}
              {currentDocument.id === 12 && "Supplier's banking details for SOLLYS BEDS (PTY) LTD"}
              {currentDocument.id === 13 && "Official bank account confirmation letter from First National Bank"}
              {currentDocument.id === 14 && "ID Document for Abrar Ahmed Limbada (ID No. 720802 5893 084)"}
              {currentDocument.id === 15 && "Additional ID Document information and certification"}
            </p>
          </div>
        </div>
      </main>
